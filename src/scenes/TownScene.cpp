...
void TownScene::keyPressEvent(QKeyEvent *event){
    ...
    case Qt::Key_Up:
        ...
        if ((playerX > 1490) && (playerX < 1560) && (playerY < 1010) && (playerY > 900)){
            resourceManager->setPreviousScene("TOWN");
            switchScene(3); // GO GRASSLAND
            break;
        }
...
void TownScene::setupScene(){
    ...
    playerX = 1670; // 預設位置 (實驗室出來)
    playerY = 1700; // 預設位置
    if (resourceManager->getPreviousScene() == "GRASSLAND") { playerX = 1520; } // 如果從草地回來
    if (resourceManager->getPreviousScene() == "GRASSLAND") { playerY = 1060; } // 如果從草地回來
        ...
        if (playerFutureRect.intersects(boxRect)){
            ...
            case 0:
                ...
                case 1:
                    ...
                    boxMessage = "從箱子獲得 2 個寶可夢球！";
                    break;
                case 2:
                    resourceManager->addPokeballCount(3);
                    boxMessage = "從箱子獲得 3 個寶可夢球！";
                    break;
                default:
                    break;
                }
...
bool TownScene::showBulletinDialog(int x, int y){
    if ((x > 1360) && (x < 1410) && (y > 1590) && (y < 1602)){
        ...
        dialogues.clear();
        dialogues << "妙蛙種子即將和傑尼龜舉行結婚典禮！" << "典禮將舉辦於 2105 年 1 月 17 日\n在成功大飯店 57 樓火焰廳" << "11:00 開始，歡迎蒞臨！";
        uiDialog->setDialogues(dialogues);
        uiDialog->setPos(anchorCenter->pos().x(), anchorCenter->pos().y());
        uiDialog->showDialogue();
        return true;
